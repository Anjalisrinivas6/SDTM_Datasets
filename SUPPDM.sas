...
if index(race,"Other"  )>0 then racoth = "OTHER: MIDDLE EASTERN " ;
SUBJINI = INITLS;

Keep studyid Domain Usubjid subjid Age Ageu siteid  brthdtc Sex1 Race1 Ethnic1 country  Dmdtc racoth subjini ;
run;
...
keep Studyid Domain Usubjid Subjid Rfstdtc Rfendtc Rfxstdtc Rfxendtc brthdtc Rficdtc Rfpendtc Dthdtc Dthfl
     Sex Race Ethnic Age Ageu Country Siteid Armcd Arm Actarmcd Actarm Dmdtc DMDY racoth subjini randn  ;
	 run;
...
SUBJINI label = 'Subject Initials'
RANDN Label = 'Randomization Number'
RACOTH Label = 'Race Other';
...
run;
 ...
 proc transpose data = fin out = Supp;
 by usubjid;
 var racoth SUBJINI RANDN;
 run;

data supp1;
Length   RDomain $2 Usubjid $18  IDVAR  QNAM $8 IDVARVAL QVAL $200  QLABEL $20  QORIG QEVAL $20 ;
set Supp;
Rdomain = 'DM';
IDVAR = '';
IDVARVAL = '';
...
QORIG = 'CRF';
QEVAL = '';
Keep  RDomain Usubjid Idvar Idvarval Qnam Qlabel Qval Qorig Qeval;
run;
